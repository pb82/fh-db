    ...
    if (null !== fields) {
      for (field in  fields) {
        if (fields.hasOwnProperty(field)) {
          query[field] = convertQueryType(fields[field]);
        }
      }
    }
  },
  ...
  if (null !== fields) {
    for (field in  fields) {
      if (fields.hasOwnProperty(field)) {
        ...
        queryField[expression] = convertType(fields[field]);
        query[field] = queryField;
      }
    }
  }
...
function getListCollectionsHandler(self, params, callback) {
  return function (err, names) {
    ...
    for (i = 0; i < names.length; i++) {
      ...
      // Need to append _ to the app db name to not include other environments
      // by accident if they have a similar name, e.g. dev & dev2

      if (!params.__dbperapp && curName.name.indexOf(params.__fhdb + "_") === -1){
        continue;
      }
